...
MODELS_PATH = os.path.join(ASSETS_PATH, 'models8')
TENSORBOARD_PATH = os.path.join(ASSETS_PATH, 'tensorboard8')
...
def load_image_disk(img_id, folder=TRAIN_PATH):
    ...
    # print("function load_image_disk() img shape:", img.shape)
    return img
...
def load_mask_disk(img_id, folder=TRAIN_MASKS_PATH, filetype='bmp'):
    ...
    # print("function load_mask_disk() mask shape:", mask.shape)
    return mask
...
for img_path in tqdm(os.listdir(TRAIN_PATH)):
    ...
    img_temp = load_image_disk(img_id)
...
for img_path in tqdm(os.listdir(TRAIN_MASKS_PATH)):
    ...
    # print('img_id:', img_id)
    mask_temp = load_mask_disk(img_id)
    if len(mask_temp.shape) == 2:
        train_masks[img_id] = np.expand_dims(mask_temp, axis=2)
    else:
        train_masks[img_id] = mask_temp
    # train_masks[img_id] = np.expand_dims(load_mask_disk(img_id),  axis=2)
...
def generate_training_batch(data, batch_size):
    while True:
        ...
        for idx, img_id in enumerate(batch_ids):
            x = get_image(img_id)
            y = get_mask(img_id)
            #print("function generate_training_batch() x shape:", x.shape)
            #print("function generate_training_batch() x shape:", y.shape)
        ...
        # print("function generate_training_batch() X shape:", X.shape)
        # print("function generate_training_batch() Y shape:", Y.shape)
        yield X, Y
...
def get_unet_1024(input_shape=(HEIGHT, WIDTH, CHANNELS), num_classes=1):
    ...
    down1, down1_res = down(48, down0)
    down2, down2_res = down(96, down1)
    down3, down3_res = down(128, down2)
    down4, down4_res = down(196, down3)

    center = Conv2D(196, (3, 3), padding='same')(down4)
    ...
    up4 = up(196, center, down4_res)
    up3 = up(128, up4, down3_res)
    up2 = up(96, up3, down2_res)
    up1 = up(48, up2, down1_res)
